...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    Sophus::SE3f Tcw_sophus = mpSLAM->TrackMonocular(cv_ptr->image, cv_ptr->header.stamp.toSec());
    Eigen::Matrix4f Tcw_eigen = Tcw_sophus.matrix();
    cv::Mat Tcw(4, 4, CV_32F);  // Create a 4x4 matrix in OpenCV format

    // Copy data from Eigen matrix to OpenCV Mat
    for (int i = 0; i < 4; ++i)
        for (int j = 0; j < 4; ++j)
